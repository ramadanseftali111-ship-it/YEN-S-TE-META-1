...
function generateSiteId() {
    ...
    if (hostname && hostname !== 'localhost' && hostname !== '127.0.0.1' && hostname !== '') {
        // www. kısmını kaldır (www.benimsite.com -> benimsite.com)
...
// Title'ı sadece BİR KEZ değiştir (sonsuz döngü engellendi)
(function() {
    var eskiDomain = 'emenkargon.online';
    var yeniDomain = window.SITE_DOMAIN;
    var titleDegistirildi = false;
    
    // Title değiştirme fonksiyonu
    function titleDegistir() {
        if (titleDegistirildi) return; // Zaten değiştirildi, tekrar yapma
        ...
        if (document.title && document.title.includes(eskiDomain)) {
            ...
            titleDegistirildi = true;
        }
    }
    
    // Sayfa yüklenince BİR KEZ çalıştır
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', titleDegistir);
    } else {
        titleDegistir();
    }
    
    // 500ms sonra bir kez daha kontrol et (Firebase'den gelen title için)
    setTimeout(titleDegistir, 500);
})();
